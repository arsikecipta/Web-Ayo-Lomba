...
<table id="customers">
  ...
  require('config.php');
  $user = $_SESSION['users'];
  $lomba_data = mysqli_query($conn,"SELECT *FROM lomba WHERE status ='waiting'");
...
?>
  <tr>
    ...
    <td><form action="submit_proses.php" method="post">
        <input type="hidden" name = "idlomba" value="<?php echo $data['id_lomba']; ?>">
       <button style="background-color:green; font-size:14px"type="accept" name = 'accept' value="accept"><p>Accept</p></button></form>

       <form action="declined.php" method="post">
        <input type="hidden" name = "idlomba" value="<?php echo $data['id_lomba']; ?>">
       <button style="font-size:14px" type="declined" name = 'declined' value="declined"><p>Decline</p></form>
