...
<table id="customers">
  ...
  require('config.php');
  $user = $_SESSION['users'];   
  $lomba_data = mysqli_query($conn,"SELECT * FROM lomba INNER JOIN users on lomba.email = users.email WHERE username= '$user'");
...
?>
  ...
  <input type="hidden" name = "idlomba" value="<?php echo $data['id_lomba']; ?>">
    ...
    <input type="hidden"  required type="email" value="<?php echo $_SESSION['email'] ?>" class="form-control" name="email" readonly placeholder="Masukkan Email Anda Saat Ini">
    <td><?php echo $data['status'] ?></td>
   <td>
    <input required type="file" name="foto">
    
    <button name = "submit" class="btn btn-primary" style="background-color: rgba(0, 140, 255, 0.925); type="submit">SUBMIT</button>
    </td>
      ...
      </table><div class="jumbotron1 text-center" style="margin-bottom:0">
    <p>&copy;Ayo Lomba</p>
<!-- </div>
